...
namespace UniUti.Test.Application.Tests.Services
{
    public class DisciplinaServiceTests
    {
        ...
        public async Task Find_by_id_should_return_disciplina()
        {
            ...
            //Action
            _service.Setup(x => x.FindById(It.IsAny<long>())).ReturnsAsync(disciplina);
            var result = await _disciplina.FindById(It.IsAny<long>());

            //Assert
            Assert.Equal(result.Id, disciplina.Id);
        }
        ...
        public async Task Delete_should_success()
        {
            ...
            var result = await _disciplina.Delete(It.IsAny<long>());
